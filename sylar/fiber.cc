...
Fiber::ptr Fiber::GetThis() {
    if (t_fiber) { // 检查当前线程的局部存储中是否已经存在一个协程对象
        return t_fiber->shared_from_this(); // 这个静态方法返回当前正在运行的协程的 std::shared_ptr<Fiber> 类型指针。
    }

    Fiber::ptr main_fiber(new Fiber);
    SYLAR_ASSERT(t_fiber == main_fiber.get());
    t_thread_fiber = main_fiber; // 这个新建的fiber被设置为主协程
 ...
 * params:cb 表示协程执行的代码
 * stacksize 协程的栈的大小，以字节为单位，如果提供的值为0 使用默认的栈大小
 * run_in_schefuler 指示该协程是否应该再调度器中运行
 */
...
void Fiber::reset(std::function<void()> cb) {
    ...
    SYLAR_ASSERT(m_state == TERM);
    m_cb = cb;
    // 获取当前上下文，如果获取失败，触发sylar-assert2断言
    if (getcontext(&m_ctx)) {
        SYLAR_ASSERT2(false, "getcontext");
    }
    ...
    // 修改上下文，使其执行mainfunc函数，这个函数是协程的实际运行函数，当协程被激活时执行
    makecontext(&m_ctx, &Fiber::MainFunc, 0);
    m_state = READY;
}
