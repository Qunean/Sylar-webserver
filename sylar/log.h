...
class LogAppender {
public:
    typedef std::shared_ptr<LogAppender> ptr; // typedef 是C++中的一个关键字，用于为现有的类定义一个新的名字 typedef existing_type new_name
    ...
    /**
     * @brief 写入日志
     * 这是一个纯虚函数，要求派生类实现该函数来处理具体的日志记录行为。参数 event 是日志事件，包含了日志的详细信息（如日志级别、消息、时间戳等）。
     */
    virtual void log(LogEvent::ptr event) = 0;

    /**
     * @brief 将日志输出目标的配置转成YAML String
     * 纯虚函数，要求派生类实现该函数，将日志输出目标的配置转换为 YAML 格式的字符串。YAML 是一种人类可读的数据序列化格式，常用于配置文件。
     */
