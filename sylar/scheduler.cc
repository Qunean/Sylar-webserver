...
/// 当前协程调度器，同一个调度器下的所有线程共享同一个实例
static thread_local Scheduler *t_scheduler = nullptr; 
/// 当前线程的调度协程，每个线程都独有一份
static thread_local Fiber *t_scheduler_fiber = nullptr;

Scheduler::Scheduler(size_t threads, bool use_caller, const std::string &name) {
    SYLAR_ASSERT(threads > 0); // 检查threads 是否大于0

    m_useCaller = use_caller;
    m_name      = name;

    if (use_caller) { // 如果当前线程作为调度器的一部分
        --threads; // thraeds-- 因为已将当前线程计算在内
        sylar::Fiber::GetThis(); // 获取当前协程调度器
...
void Scheduler::start() {
    ...
    SYLAR_ASSERT(m_threads.empty()); // 检查m_treads 线程池是否为空了
    // 创建线程池
